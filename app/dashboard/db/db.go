...
func InitDB() *sql.DB {
	...
	_, err = db.Exec(`
		...
		CREATE TABLE IF NOT EXISTS user_notification_settings (
			...
			notify_ticket_update BOOLEAN NOT NULL DEFAULT true,
			notify_ticket_comment BOOLEAN NOT NULL DEFAULT true,
	...
	// В PostgreSQL индексы создаются отдельным оператором
...
func LogAction(db *sql.DB, userID int, action, details string) {
	_, err := ExecuteQuery(db, "INSERT INTO logs (user_id, action, details) VALUES ($1, $2, $3)",
